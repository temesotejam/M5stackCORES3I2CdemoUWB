...
## CoreS3 v1.1.1：外付けプルアップへ統一

ユーザーの実測では2DKのTP18は3.0 Vでした。v1.1.0で有効だったPORT Aの内蔵プルアップを無効化し、CoreS3の3.3 Vへ引き上げる経路を除きます。初期化・100/400 kHz切替の両方に適用します。2DKのBINはv5のままで、再書き込み不要です。

- 電源を切り、外付け抵抗の電源側をCoreS3の3V3から外してTP18へ移します。**TP18–2 kΩ–SDA、TP18–別の2 kΩ–SCL**です。
- CoreS3の3V3とTP18を直接つながないでください。GNDは共通、双方のUSB給電を使います。
- 外付けプルアップが必須です。v1.1.1ではPORT A未接続時の電圧は不定で、idle=1/1を期待値にしません。
- CoreS3を書き換え、ログの `fw=1.1.1` を確認。2DK起動後にCLEARし、手で配線を押さえず100 kHz、続いて400 kHzでエラー増加の有無を確認します。

QN9090の入力High上限はI/O電源電圧です。[NXPデータシート Table 17](https://www.nxp.jp/docs/en/nxp/data-sheets/QN9090%28T%29QN9030%28T%29.pdf)。ESP32-S3は3.3 V動作時のHigh入力下限が0.75×3.3=2.475 Vで、3.0 Vはこの電圧条件を満たします。[ESP32-S3データシート Table 5-4](https://www.espressif.com/sites/default/files/documentation/esp32-s3_datasheet_en.pdf)。TP18が異なる電圧の別個体では双方の入力条件を再確認してください。

配線を押さえると受信が改善したという報告から、接触不良をまず疑います。今回の電圧設定変更で接触不良が直るわけではありません。線・コネクタ・抵抗接続部も固定または交換してください。
...
外部PORT AのみをEspressif ESP-IDFのI2Cドライバへ変更。M5Unifiedの内部バスと別コントローラであることを検査してから外部バスを引き継ぎます。これにより旧版のbool型にまとめられていたエラーを数値で取得します。旧版との通信実装比較も兼ねる診断変更です。後に2DK v5との100 kHz受信を確認しました。16バイト形式はv1互換です。2DKのUSBログが必要な場合は下記の診断版v5へ書き換えます。
...
- `probe`：最後に実行したPROBEの結果を保持します。各DIAGで再試行する意味ではなく、read=VALIDの間も過去のNO_ACKが残ることがあります。CLEARでもこの値は消えません。

100 kHzで約10秒動かし、PROBEとDIAGの行を採取してください。v1.1.0は内部の弱いプルアップが有効でした。現在のv1.1.1は無効のため、電圧を合わせた外付け抵抗が必須です。
