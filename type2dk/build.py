...
python build.py --zig /path/to/zig --sdk-root /path/to/uwbiot-top
"""
...
name='2dk_uart_tx_v1' if a.uart else '2dk_i2c_diag_v5'
...
subprocess.run([a.zig,'cc','-target','thumb-freestanding-eabi','-mcpu=cortex_m4',
 ...
 '-I'+str(root.parent/'include'),str(root/('src/uart_tx.c' if a.uart else 'src/main.c')),str(root/'src/startup.c'),'-Wl,-T,'+str(root/'src/link.ld'),
...
(root/('UART_SHA256SUMS.txt' if a.uart else 'SHA256SUMS.txt')).write_text(hashlib.sha256(b).hexdigest()+'  '+out.name+'\n')
